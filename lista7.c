...
void imprimirPlanoAlimentar(struct Alimento alimentos[], int tamanho, const char* refeicao, FILE *file) {
    int caloriasTotais = 0;
    fprintf(file, "%s\n", refeicao);
    for (int i = 0; i < tamanho; i++) {
        fprintf(file, "%-20s %6.2f %s  %4d Kcal\n", alimentos[i].nome, alimentos[i].quantidade, alimentos[i].unidade, alimentos[i].calorias);
        caloriasTotais += alimentos[i].calorias;
    }
    fprintf(file, "Calorias totais: %d Kcal\n\n", caloriasTotais);
}

// Função para imprimir a lista de compras consolidada
void imprimirLista(struct ItemCompra lista[], int tamanho, FILE *file) {
    for (int i = 0; i < tamanho; i++) {
        fprintf(file, "%d. %-20s %6.2f %s\n", i + 1, lista[i].nome, lista[i].quantidade, lista[i].unidade);
    }
}
...
int main() {
    ...
    struct Alimento carnes[] = {
        ...
        {"Patinho moida", 150.0, "g", 146},
        {"Carne Cozida", 150.0, "g", 116},
        {"Carne de Porco", 150.0, "g", 130},
        {"Patinho moida", 150.0, "g", 118}
    };
    ...
    for (int dia = 0; dia < 7; dia++) {
        ...
        // Café da Manhã
        imprimirPlanoAlimentar(cafe_da_manha, sizeof(cafe_da_manha) / sizeof(cafe_da_manha[0]), "12:00 - Café da Manhã", file);
        ...
        imprimirPlanoAlimentar(almoco, sizeof(almoco) / sizeof(almoco[0]), "14:00 - Almoço", file);
        ...
        fprintf(file, "15:00 - Café puro\n\n");

        // Lanche da Tarde
        imprimirPlanoAlimentar(lanche_da_tarde, sizeof(lanche_da_tarde) / sizeof(lanche_da_tarde[0]), "18:30 - Lanche da Tarde", file);
        ...
        // Ceia
        imprimirPlanoAlimentar(ceia, sizeof(ceia) / sizeof(ceia[0]), "22:00 - Ceia", file);
    ...
    fprintf(file, "Lista de Compras:\n");
    imprimirLista(listaDeCompras, tamanho_lista, file);
